...
" 2>/dev/null || true)
    ...
    if [[ "$PUSH" == true ]]; then
        ...
        git commit -m "Add $EXPERIMENT results ($GPU_INFO${SUFFIX})" || true
        git stash --include-untracked || true
        git pull --rebase || {
            echo "Rebase failed (likely squashed history) — resetting to remote and re-committing"
...
trap 'echo "ERROR: Experiment failed at line $LINENO"; archive_and_push "_failed"' ERR
...
if [[ "$ARCHIVE_ONLY" == false ]]; then
    ...
    # --- Run eval ---
    if [[ "$EVAL" == true ]]; then
        echo "Running evaluation (${NUM_GPUS} GPU(s))..."
        local_eval_args=("${EVAL_ARGS[@]}")
        if [[ "$USE_NAMED_ARTIFACTS" == true ]]; then
            local_eval_args+=("--checkpoint" "$ARTIFACTS_DIR/checkpoint" "--report" "$ARTIFACTS_DIR/eval_report.json")
        fi
        if [[ "$NUM_GPUS" -gt 1 ]]; then
            torchrun --nproc_per_node="$NUM_GPUS" "$SCRIPT_DIR/eval.py" "${local_eval_args[@]}"
        else
            python3 "$SCRIPT_DIR/eval.py" "${local_eval_args[@]}"
        fi
    fi
else
